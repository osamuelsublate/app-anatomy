...
import type { Diagram, DiagEdge, DiagNode, EdgePoint } from "./types.js";
...
export interface MarkupVisualState {
...
function squirclePath(x: number, y: number, w: number, h: number, r: number): string {
...
function nodeRadius(w: number, h: number): number {
  return Math.min(NODE_RADIUS, w / 2, h / 2);
}

function nodeSquircle(x: number, y: number, w: number, h: number): string {
  return squirclePath(x, y, w, h, nodeRadius(w, h));
}

/** Id seguro para defs por nó (os IDs do modelo já são [A-Za-z0-9_-]). */
function defsSafeId(value: string): string {
...
function arrowTriangle(sx: number, sy: number, ex: number, ey: number, size = 9): string {
 ...
 */
function glassDefs(theme: ResolvedTheme): string {
...
function blockMarkup(blockId: string, theme: ResolvedTheme): string {
...
export function paletteMarkup(query = "", theme: ResolvedTheme = LIGHT_THEME): string {
  ...
  const categories = CATEGORIAS.map((category) => {
    const visibleBlocks = category.blocos.filter((block) =>
      `${block.nome} ${block.desc} ${category.nome}`.toLowerCase().includes(normalizedQuery),
    );
...
export function isBackdropNode(node: DiagNode): boolean {
  return nodeWidth(node) * nodeHeight(node) >= DEFAULT_NODE_AREA * 2.2;
}

export function nodeMarkup(node: DiagNode, theme: ResolvedTheme = LIGHT_THEME): string {
  ...
  if (node.def === TEXT_BLOCK_DEF) {
    ...
    content = textLines
      .map(
        (line, index) =>
          `<text x="${w / 2}" y="${h / 2 + (index - (textLines.length - 1) / 2) * 18}" text-anchor="middle" dominant-baseline="central" font-family="${FONT}" font-size="13" font-weight="500" fill="${labelColor}">${escapeXml(line)}</text>`,
      )
      .join("");
    ...
    const labelTexts = lines
      .map(
        (line, index) =>
          `<text x="${w / 2}" y="${labelYs[index]!}" text-anchor="middle" font-family="${FONT}" font-size="13" font-weight="600" fill="${labelColor}">${escapeXml(line)}</text>`,
      )
      .join("");
...
function polyPathD(pts: readonly EdgePoint[]): string {
...
function orthoPathD(pts: readonly EdgePoint[], radius = 9): string {
  if (pts.length < 3) return polyPathD(pts);
...
function curvePathD(pts: readonly EdgePoint[]): { d: string; tangent: EdgePoint; mid: EdgePoint } {
  if (pts.length === 2) {
...
export interface EdgeVisual {
  /** Path completo para a área de clique. */
  dHit: string;
  /** Path visível (linha, aparada antes da seta quando reta/ortogonal). */
  dLine: string;
  /** Triângulo da seta, orientado pelo fim do trajeto. */
  arrow: string;
  /** Ponto médio do trajeto (posição do rótulo). */
  mid: EdgePoint;
}

/** Constrói os paths SVG de uma rota; compartilhado entre documento e overlay. */
export function edgeVisual(route: EdgeRoute): EdgeVisual {
  const pts = route.pts;
...
export function edgeMarkup(
  ...
  theme: ResolvedTheme = LIGHT_THEME,
...
export function documentMarkup(diagram: Diagram, theme: ResolvedTheme = LIGHT_THEME): string {
...
export function presentationDiagram(diagram: Diagram, revealedCount: number): Diagram {
...
export function interactionMarkup(
  diagram: Diagram,
  visualState: MarkupVisualState,
  theme: ResolvedTheme = LIGHT_THEME,
): string {
  if (visualState.presentation) return "";
  ...
  const guides = (visualState.guides ?? []).map((guide) =>
    guide.axis === "x"
      ? `<line class="alignment-guide" x1="${guide.position}" y1="${guide.from}" x2="${guide.position}" y2="${guide.to}" stroke="${theme.accent}"/>`
      : `<line class="alignment-guide" x1="${guide.from}" y1="${guide.position}" x2="${guide.to}" y2="${guide.position}" stroke="${theme.accent}"/>`,
  ).join("");
  ...
  const edgeHandles = selectedEdge?.points?.length
    ? `<g class="edge-points">${selectedEdge.points.map((pt, index) =>
      `<circle class="edge-point" data-edge-point="${escapeXml(selectedEdge.id)}" data-point-index="${index}" cx="${pt.x}" cy="${pt.y}" r="6.5" tabindex="0" role="button" aria-label="Ponto de passagem ${index + 1}" fill="${theme.surface}" stroke="${theme.accent}" stroke-width="2"/>`).join("")}</g>`
    : "";
  return ports + `<g class="alignment-guides">${guides}</g>` + edgeHandles;
}

export function overlayMarkup(diagram: Diagram, visualState: MarkupVisualState, theme: ResolvedTheme = LIGHT_THEME): string {
  const nodeIndex = new Map(diagram.nodes.map((node) => [node.id, node]));
  const selectedNode =
    visualState.selection?.kind === "node" ? nodeIndex.get(visualState.selection.id) : undefined;
  const selectedEdge =
    visualState.selection?.kind === "edge"
      ? diagram.edges.find((edge) => edge.id === visualState.selection?.id)
      : undefined;
  let selection = "";
  if (selectedNode) {
...
export function buildExportSvg(diagram: Diagram): ExportSvg | null {
