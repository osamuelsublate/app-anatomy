...
const G: Readonly<Record<string, string>> = Object.freeze({
  // ---------- Pessoas ----------
...
export const CATEGORY_GLYPHS: Readonly<Record<string, string>> = Object.freeze({
  pessoas: "usuario",
...
export function hasGlyph(defId: string): boolean {
  return defId in G;
}

function glyphBody(defId: string, color: string): string {
 ...
 */
export function glyphMarkup(defId: string, x: number, y: number, size: number, color: string): string {
  const scale = size / 24;
...
export function glyphSvg(defId: string, size: number, className = ""): string {
