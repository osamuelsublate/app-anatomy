...
import type { EdgeLineStyle, PortSide } from "./types.js";
...
const cats = $<HTMLElement>("#cats");
const search = $<HTMLInputElement>("#search");
const hint = $<HTMLElement>("#hint");
const labelEditor = $<HTMLInputElement>("#labelEditor");
const fileInput = $<HTMLInputElement>("#fileInput");
const saveStatus = $<HTMLElement>("#saveStatus");
const announcer = $<HTMLElement>("#announcer");
const documentSelect = $<HTMLSelectElement>("#documentSelect");
const documentTitle = $<HTMLInputElement>("#documentTitle");
const templateSelect = $<HTMLSelectElement>("#templateSelect");
const quickPicker = $<HTMLElement>("#quickPicker");
const quickSearch = $<HTMLInputElement>("#quickSearch");
const quickResults = $<HTMLElement>("#quickResults");
const shortcuts = $<HTMLElement>("#shortcutsOverlay");
const fileMenu = $<HTMLElement>("#fileMenu");
const fileMenuButton = $<HTMLButtonElement>("#btnFileMenu");
const colorPanel = $<HTMLElement>("#colorPanel");
const colorCustom = $<HTMLInputElement>("#colorCustom");
const contextMenu = $<HTMLElement>("#contextMenu");
const btnPalette = $<HTMLButtonElement>("#btnPalette");
...
let currentTheme: ResolvedTheme = storedTheme() === "dark" ? DARK_THEME : LIGHT_THEME;
...
let portDrag: { from: string; fromSide: PortSide | null; moved: boolean } | null = null;
/**
 ...
 */
let edgeDrag: { id: string; x: number; y: number; index: number | null } | null = null;
let panning: { sx: number; sy: number; x: number; y: number } | null = null;
let editing: EditorTarget | null = null;
let activePointerId: number | null = null;
let activeGuides: readonly AlignmentGuide[] = [];
let quickFrom: string | null = null;
let quickAt: { x: number; y: number } | null = null;
...
const app = createApplicationModel(() => localStorage, () => Math.random().toString(36).slice(2, 10), {
  ...
  onStatus(message, isError) {
    ...
    if (isError || (!message.startsWith("Salv") && !message.startsWith("Carregando"))) announce(message);
  },
...
function applyViewport(): void {
  ...
  $<HTMLElement>("#btnZoomReset").textContent = `${Math.round(viewport.zoom * 100)}%`;
}
...
function updateColorPanel(): void {
  ...
  for (const swatch of colorPanel.querySelectorAll<HTMLElement>("[data-color]")) {
    swatch.classList.toggle("active", (swatch.dataset["color"] ?? "") === color);
  }
  if (color) colorCustom.value = color;
}

function renderForMode(mode: UpdateMode): void {
  const diagram = displayedDiagram();
  if (mode === "transient" && drag) renderer.patchDrag(diagram, visualState(), drag.id, currentTheme);
  else if (mode === "transient" && (resizing || edgeDrag)) renderer.renderFull(diagram, visualState(), currentTheme);
  else if (mode === "transient" && portDrag) renderer.renderOverlay(diagram, visualState(), currentTheme);
  else if (mode === "transient") renderer.renderUi(diagram, visualState(), currentTheme);
  else renderer.renderFull(diagram, visualState(), currentTheme);
  applyViewport();
  updateColorPanel();
...
function applyTheme(name: ThemeName, persist = true): void {
  ...
  if (persist) {
    try { localStorage.setItem(THEME_STORAGE_KEY, name); }
    catch { status.show("Tema aplicado, mas a preferência não pôde ser salva.", true); }
  }
  cats.innerHTML = paletteMarkup(search.value, currentTheme);
  renderFull();
  $<HTMLElement>("#btnTheme").setAttribute(
    "aria-label",
    name === "dark" ? "Usar tema claro" : "Usar tema escuro",
  );
}

function enterPresentation(): void {
  if (presentation || app.file.nodes.length === 0) return;
  presentation = {
    revealed: 1,
    viewport: { ...viewport },
...
function revealPresentation(delta: number): void {
...
function beginGesture(): void { gestureRect = canvas.getBoundingClientRect(); }
function endGesture(): void { gestureRect = null; activeGuides = []; }
function refreshGesture(): void { if (gestureRect) gestureRect = canvas.getBoundingClientRect(); }

function addNode(definitionId: string, clientX: number, clientY: number, offset = 0): string | null {
...
function closeEditor(save: boolean): void {
  ...
  if (save && target) app.rename(target, labelEditor.value.trim());
}

const pointerMoves = createFrameCoalescer(
  ({ x, y, free }: { x: number; y: number; free: boolean }) => {
    ...
    if (drag) {
      ...
      if (resizing.corner.includes("e")) nextW = normalizeDiagramNodeSize(pointer.x - resizing.x, "width");
      if (resizing.corner.includes("s")) nextH = normalizeDiagramNodeSize(pointer.y - resizing.y, "height");
      ...
      if (edgeDrag.index === null) {
        ...
        if (Math.hypot(target.x - edgeDrag.x, target.y - edgeDrag.y) * viewport.zoom < 4) return;
 ...
 */
function edgeInsertIndex(edgeId: string, x: number, y: number): number | null {
...
function closeQuickPicker(): void {
  quickPicker.hidden = true;
  quickFrom = null;
  quickAt = null;
}

function renderQuickResults(query = ""): void {
...
function placeQuickPicker(screenPoint: { x: number; y: number }): void {
  const wrap = canvasWrap.getBoundingClientRect();
...
function openQuickPicker(from: string, clientX?: number, clientY?: number): void {
  quickFrom = from;
  quickAt = null;
...
function openAddPicker(clientX: number, clientY: number): void {
  quickFrom = null;
...
function quickCreate(definitionId: string): void {
  if (!quickFrom) {
    const at = quickAt;
    closeQuickPicker();
    if (!at) return;
  ...
  if (!source) return closeQuickPicker();
...
let hoveredPortsId: string | null = null;

/** Mostra as portas do bloco sob o cursor (como o draw.io faz ao pairar). */
function updateHoverPorts(target: Element | null): void {
  const element = target?.closest<HTMLElement>("[data-node-id],[data-ports-for],[data-port-node]");
  const id = element?.dataset["nodeId"] ?? element?.dataset["portsFor"] ?? element?.dataset["portNode"] ?? null;
  if (id === hoveredPortsId) {
    if (id) interactionLayer.querySelector(`[data-ports-for="${CSS.escape(id)}"]`)?.classList.add("hover");
    return;
  }
  if (hoveredPortsId) {
...
function finishPointerGesture(event: PointerEvent): void {
  ...
  if (edgeDrag) {
    if (edgeDrag.index !== null) app.finishTransient();
    edgeDrag = null;
    renderFull();
  }
  if (portDrag) {
    ...
    const target = document.elementFromPoint(event.clientX, event.clientY)?.closest<HTMLElement>("[data-node-id],[data-port-node]");
    portDrag = null;
    ...
    if (moved && targetId) {
      ...
      const toSide = (target?.dataset["portNode"] ? target.dataset["portSide"] ?? null : null) as PortSide | null;
      app.connectNodes(source, targetId, {
        ...(fromSide ? { fromSide } : {}),
        ...(toSide ? { toSide } : {}),
      });
    } else if (!moved) openQuickPicker(source, event.clientX, event.clientY);
  }
  panning = null;
...
function toggleFileMenu(open = fileMenu.hidden): void {
  fileMenu.hidden = !open;
  fileMenuButton.setAttribute("aria-expanded", String(open));
  if (open) fileMenu.querySelector<HTMLElement>("button,select")?.focus();
  else fileMenuButton.focus();
}

function paletteHidden(): boolean {
...
function setPaletteHidden(hidden: boolean, persist = true): void {
  ...
  refreshGesture();
  if (!persist) return;
  try { localStorage.setItem(PALETTE_STORAGE_KEY, hidden ? "hidden" : "visible"); }
  catch { /* preferência não persistida; segue só nesta sessão */ }
}

function storedPaletteHidden(): boolean {
  try { return localStorage.getItem(PALETTE_STORAGE_KEY) === "hidden"; }
  catch { return false; }
}

// ---- Menu de contexto do canvas ----
type ContextMenuEntry =
  | { kind: "action"; id: string; label: string; run: () => void; danger?: boolean; disabled?: boolean }
  | { kind: "separator" }
  | { kind: "swatches"; nodeId: string };
...
function contextSwatchRow(nodeId: string): HTMLElement {
  ...
  const swatch = (color: string, className: string, label: string): HTMLButtonElement => {
    const button = document.createElement("button");
    button.type = "button";
    ...
    if (color) button.style.setProperty("--sw", color);
    button.classList.toggle("active", color === current);
    return button;
  };
  row.append(
    swatch("", "swatch swatch-clear", "Sem cor (vidro transparente)"),
    ...MENU_SWATCH_COLORS.map((color) => swatch(color, "swatch", color)),
  );
  return row;
}

function openContextMenu(entries: ContextMenuEntry[], clientX: number, clientY: number): void {
  contextActions = new Map();
  contextMenu.replaceChildren(...entries.map((entry) => {
    ...
    if (entry.kind === "swatches") return contextSwatchRow(entry.nodeId);
    const button = document.createElement("button");
    button.type = "button";
  ...
  contextMenu.querySelector<HTMLElement>("button[role=menuitem]:not(:disabled)")?.focus();
}

function nodeContextEntries(id: string, clientX: number, clientY: number): ContextMenuEntry[] {
...
const EDGE_LINE_OPTIONS: readonly { value: EdgeLineStyle; label: string }[] = [
  { value: "straight", label: "Linha reta" },
  { value: "ortho", label: "Linha ortogonal" },
  { value: "curved", label: "Linha curva" },
];

function edgeContextEntries(id: string): ContextMenuEntry[] {
  const edge = app.file.edges.find((item) => item.id === id);
  const currentLine: EdgeLineStyle = edge?.line ?? "straight";
  ...
  return [
    ...
    { kind: "separator" },
    ...EDGE_LINE_OPTIONS.map((option): ContextMenuEntry => ({
      kind: "action",
...
function canvasContextEntries(clientX: number, clientY: number): ContextMenuEntry[] {
  return [
...
function openShortcuts(): void {
  ...
  $<HTMLElement>("#shortcutsTitle").focus();
}
...
async function useSelectedTemplate(insert: boolean): Promise<void> {
...
function firstPaletteDefinition(): string | null {
  return cats.querySelector<HTMLElement>("[data-def]")?.dataset["def"] ?? null;
}

function insertPaletteResult(): void {
  ...
  if (selected) {
    ...
    if (id) announce("Bloco criado e conectado.");
  } else {
    const rect = canvas.getBoundingClientRect();
...
function bindControls(): void {
  ...
  cats.addEventListener("click", (event) => {
    const block = (event.target as Element).closest<HTMLElement>("[data-def]");
    if (!block?.dataset["def"]) return;
    const rect = canvas.getBoundingClientRect();
  ...
  search.addEventListener("keydown", (event) => {
    if (event.key === "Enter") { event.preventDefault(); insertPaletteResult(); }
  });
  colorPanel.addEventListener("click", (event) => {
    const swatch = (event.target as Element).closest<HTMLElement>("[data-color]");
    const selected = app.ui.selection;
    if (!swatch || selected?.kind !== "node") return;
  ...
  colorCustom.addEventListener("input", () => {
    const selected = app.ui.selection;
    if (selected?.kind === "node") app.setNodeColor(selected.id, colorCustom.value, "transient");
  });
  colorCustom.addEventListener("change", () => {
    const selected = app.ui.selection;
    if (selected?.kind === "node") app.setNodeColor(selected.id, colorCustom.value);
  });
  ...
  canvasWrap.addEventListener("contextmenu", (event) => {
    ...
    closeQuickPicker();
    closeEditor(false);
    if (!fileMenu.hidden) toggleFileMenu(false);
    const target = event.target as Element;
    if (!target.closest("#canvas")) return;
    const nodeId = target.closest<HTMLElement>("[data-node-id]")?.dataset["nodeId"]
      ?? target.closest<HTMLElement>("[data-port-node]")?.dataset["portNode"]
      ?? target.closest<HTMLElement>("[data-resize-node]")?.dataset["resizeNode"];
  ...
  contextMenu.addEventListener("click", (event) => {
    ...
    if (swatch) {
      const selected = app.ui.selection;
      if (selected?.kind === "node") app.setNodeColor(selected.id, swatch.dataset["color"] || null);
      closeContextMenu();
      return;
    }
    const item = target.closest<HTMLElement>("button[data-action]");
    if (!item?.dataset["action"] || item.hasAttribute("disabled")) return;
  ...
  contextMenu.addEventListener("keydown", (event) => {
    ...
    event.preventDefault();
    const items = [...contextMenu.querySelectorAll<HTMLElement>("button[role=menuitem]:not(:disabled)")];
  ...
  fileMenu.addEventListener("click", (event) => {
    if ((event.target as Element).closest("button[role=menuitem]")) toggleFileMenu(false);
  });
  document.addEventListener("pointerdown", (event) => {
    if (!fileMenu.hidden && !(event.target as Element).closest(".file-menu")) toggleFileMenu(false);
    if (!contextMenu.hidden && !(event.target as Element).closest("#contextMenu")) closeContextMenu();
  });
  canvas.addEventListener("pointerdown", (event) => {
    ...
    if (activePointerId !== null && event.pointerId !== activePointerId) return;
    const target = event.target as Element;
    const handle = target.closest<HTMLElement>("[data-resize-node]");
    const port = target.closest<HTMLElement>("[data-port-node]");
    const node = target.closest<HTMLElement>("[data-node-id]");
    const edge = target.closest<HTMLElement>("[data-edge-id]");
    const edgePointHandle = target.closest<HTMLElement>("[data-edge-point]");
    if (handle?.dataset["resizeNode"] && handle.dataset["resizeCorner"] && event.button === 0) {
      ...
      resizing = {
        id,
        corner: handle.dataset["resizeCorner"] as "nw" | "ne" | "se" | "sw",
        x: item.x,
        y: item.y,
        w: nodeWidth(item),
        h: nodeHeight(item),
      ...
      beginGesture();
      const fromSide = (port.dataset["portSide"] ?? null) as PortSide | null;
  ...
  canvas.addEventListener("pointermove", (event) => {
    ...
    if (!presentation && activePointerId === null) updateHoverPorts(event.target as Element);
    if (activePointerId !== null && event.pointerId !== activePointerId) return;
    if (drag || resizing || portDrag || edgeDrag) pointerMoves.schedule({ x: event.clientX, y: event.clientY, free: event.altKey });
    else if (panning) {
  ...
  canvas.addEventListener("click", (event) => {
    const template = (event.target as Element).closest<HTMLElement>("[data-template-id]");
  ...
  canvas.addEventListener("keydown", (event) => {
    const port = (event.target as Element).closest<HTMLElement>("[data-port-node]");
    const template = (event.target as Element).closest<HTMLElement>("[data-template-id]");
    if ((event.key === "Enter" || event.key === " ") && port?.dataset["portNode"]) {
      ...
      templateSelect.value = template.dataset["templateId"];
      void useSelectedTemplate(false);
    } else if ((event.target as Element).closest("[data-node-id]") && event.key === "Enter") {
      openSelectedEditor();
    }
  });
  canvas.addEventListener("dblclick", (event) => {
    const pointHandle = (event.target as Element).closest<HTMLElement>("[data-edge-point]");
    ...
    const element = (event.target as Element).closest<HTMLElement>("[data-node-id],[data-edge-id]");
  ...
  canvas.addEventListener("dragover", (event) => {
    if (event.dataTransfer?.types.includes("text/anatomy-block")) event.preventDefault();
  });
  canvas.addEventListener("drop", (event) => {
    const definitionId = event.dataTransfer?.getData("text/anatomy-block");
    if (definitionId) { event.preventDefault(); addNode(definitionId, event.clientX, event.clientY); }
  });
  ...
  quickSearch.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      const first = quickResults.querySelector<HTMLElement>("[data-def]");
      if (first?.dataset["def"]) quickCreate(first.dataset["def"]);
    } else if (event.key === "Escape") closeQuickPicker();
  });
  quickResults.addEventListener("click", (event) => {
    const block = (event.target as Element).closest<HTMLElement>("[data-def]");
    if (block?.dataset["def"]) quickCreate(block.dataset["def"]);
  });
  document.addEventListener("keydown", (event) => {
    if (presentation) {
      if (event.key === "Escape") { event.preventDefault(); exitPresentation(); }
      else if (event.key === "ArrowRight") { event.preventDefault(); revealPresentation(1); }
      else if (event.key === "ArrowLeft") { event.preventDefault(); revealPresentation(-1); }
      return;
    }
    if (isTextEntryTarget(event.target)) return;
    const modifier = event.ctrlKey || event.metaKey;
    const key = event.key.toLowerCase();
    if (modifier && key === "z" && !event.shiftKey) { event.preventDefault(); app.undo(); }
    else if ((modifier && key === "y") || (modifier && event.shiftKey && key === "z")) { event.preventDefault(); app.redo(); }
      ...
      if (selected) {
        ...
        if (id) openEditor(app.editorTarget({ kind: "node", id })!);
      }
    } else if (modifier && key === "d") {
      event.preventDefault();
      const selected = app.ui.selection?.kind === "node" ? app.nodeIndex.get(app.ui.selection.id) : null;
      if (selected) app.duplicateNode(selected.id, selected.x + 24, selected.y + 24);
      ...
      else if (!shortcuts.hidden) closeShortcuts();
      else if (!fileMenu.hidden) toggleFileMenu(false);
      else { closeQuickPicker(); app.cancelConnection(); app.select(null); }
      ...
      if (selected) app.connectNode(selected);
    }
  });
  $<HTMLElement>("#btnTheme").addEventListener("click", () => {
    applyTheme(currentTheme.name === "light" ? "dark" : "light");
  });
  $<HTMLElement>("#btnPresent").addEventListener("click", enterPresentation);
  $<HTMLElement>("#btnUndo").addEventListener("click", () => app.undo());
  $<HTMLElement>("#btnRedo").addEventListener("click", () => app.redo());
  $<HTMLElement>("#btnZoomIn").addEventListener("click", () => setZoom(viewport.zoom * 1.2));
  $<HTMLElement>("#btnZoomOut").addEventListener("click", () => setZoom(viewport.zoom / 1.2));
  $<HTMLElement>("#btnZoomReset").addEventListener("click", () => setZoom(1));
  $<HTMLElement>("#btnFit").addEventListener("click", fitView);
  $<HTMLElement>("#btnCloseShortcuts").addEventListener("click", closeShortcuts);
  bindBrowserExports(
    { png: $("#btnPng"), svg: $("#btnSvg"), json: $("#btnSave") },
    { diagram: () => app.file, file: () => app.exportFile() },
    status.reportExport,
  );
  $<HTMLElement>("#btnOpen").addEventListener("click", () => fileInput.click());
  $<HTMLElement>("#btnNewDocument").addEventListener("click", () => { if (app.createDocument()) { renderFull(); fitView(); } });
  $<HTMLElement>("#btnNewFromTemplate").addEventListener("click", () => void useSelectedTemplate(false));
  $<HTMLElement>("#btnInsertTemplate").addEventListener("click", () => void useSelectedTemplate(true));
  $<HTMLElement>("#btnDeleteDocument").addEventListener("click", () => {
    if (confirm(`Excluir "${app.file.meta.title}"?`)) { app.deleteCurrentDocument(); renderFull(); fitView(); }
  });
  $<HTMLElement>("#btnClear").addEventListener("click", () => {
    if (app.file.nodes.length && confirm("Apagar o diagrama inteiro?")) app.clear();
  });
  documentSelect.addEventListener("change", () => {
    if (app.switchDocument(documentSelect.value)) { cascade = 0; renderFull(); fitView(); }
  });
  documentTitle.addEventListener("change", () => { app.setTitle(documentTitle.value); refreshDocuments(); });
  documentTitle.addEventListener("keydown", (event) => { if (event.key === "Enter") documentTitle.blur(); });
  fileInput.addEventListener("change", () => {
    const file = fileInput.files?.[0];
    if (file) void readDiagramFile(file).then((imported) => {
      if (imported.ok && app.createDocument(imported.file)) {
        renderFull();
        fitView();
        status.show("Arquivo importado.");
      } else status.show(imported.message ?? "Não foi possível importar o arquivo.", true);
    });
    fileInput.value = "";
  });
}
