...
function jitteredSegment(rand, x1, y1, x2, y2, amp) {
...
export function roughRectOutline(x, y, w, h, seedKey) {
...
export function fillRectPath(x, y, w, h, r) {
    return [
        `M ${x + r} ${y}`,
        `H ${x + w - r}`,
        `Q ${x + w} ${y} ${x + w} ${y + r}`,
...
export function roughLine(x1, y1, x2, y2, seedKey) {
    const rand = mulberry32(hashSeed(seedKey));
    const dx = x2 - x1;
    const dy = y2 - y1;
    ...
    for (let pass = 0; pass < 2; pass++) {
        ...
        parts.push(`M ${(x1 + j() * 0.5).toFixed(1)} ${(y1 + j() * 0.5).toFixed(1)} Q ${mx.toFixed(1)} ${my.toFixed(1)} ${(x2 + j() * 0.5).toFixed(1)} ${(y2 + j() * 0.5).toFixed(1)}`);
    }
    return parts.join(" ");
}
/** Ponta de seta (duas hastes) no ponto final, apontando de (x1,y1) para (x2,y2). */
export function arrowHead(x1, y1, x2, y2, seedKey, size = 13) {
