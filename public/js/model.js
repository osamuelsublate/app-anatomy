...
import { deleteStoredDocument, documentKey, isSafeDocumentId, migrateLegacyDocument, persistDocument, readDocumentResult, readRegistry, reserveDocumentId, setActiveDocument, } from "./document-storage.js";
import { createEmptyDiagramFile, DEFAULT_DIAGRAM_TITLE, FORMAT_LIMITS, hasDiagramCapacity, insertIntoDiagram, isSafeDiagramId, normalize, normalizeDiagramCoordinate, normalizeDiagramEdgeLine, normalizeDiagramEdgeSide, normalizeDiagramLabel, normalizeDiagramNodeColor, normalizeDiagramNodeSize, normalizeDiagramTitle, prepareDiagramFile, } from "./format.js";
...
export const COURSE_TEMPLATES = Object.freeze(DIAGRAM_TEMPLATES.filter(({ id }) => id.startsWith("aula-")));
...
export async function loadDiagramTemplate(template, request = (url) => fetch(url)) {
    try {
        ...
        if (!normalized.ok)
            throw new Error("template normalization failed");
...
function cappedPush(stack, snapshot) {
    stack.push(snapshot);
    if (stack.length > MODEL_HISTORY_LIMIT)
        stack.shift();
}
export function createModel(initialState, options = {}) {
    const normalizedInitial = normalize(initialState.doc);
    if (!normalizedInitial.ok)
        throw new TypeError("initial document is invalid");
    let state = clone({ ...initialState, doc: normalizedInitial.file });
    let nodeIndex = new Map();
    const undoStack = [];
    const redoStack = [];
    let transientBefore = null;
    let saveHandle = null;
    let dirty = options.initialDirty ?? false;
    const persistence = options.persistence;
    const schedule = persistence?.schedule ??
        ((callback, delayMs) => globalThis.setTimeout(callback, delayMs));
    const cancel = persistence?.cancel ??
    ...
    function restoreDoc(snapshot) {
        state.doc = deserialize(snapshot);
        refreshNodeIndex();
    }
    function update(mutator, { mode }) {
        ...
        if (mutatedDoc !== beforeDoc) {
            ...
            state.doc = normalized.ok ? normalized.file : deserialize(beforeDoc);
        }
        refreshNodeIndex();
        ...
        if (mode === "transient") {
            if (afterState === beforeState)
                return false;
            if (afterDoc !== beforeDoc && transientBefore === null)
                transientBefore = beforeDoc;
            if (transientBefore === afterDoc)
                transientBefore = null;
            notify(mode);
            return true;
        }
    ...
    return {
        ...
        get nodeIndex() {
            return new Map([...nodeIndex].map(([id, node]) => [id, clone(node)]));
        },
        get canUndo() {
...
export function createFrameCoalescer(apply, scheduleFrame, cancelFrame) {
    let pending = null;
    let handle = null;
    const flush = () => {
        if (handle !== null)
...
export function createApplicationModel(storageSource, createId, callbacks = {}) {
    let storage;
    try {
    ...
    const recoverableClaims = new Map();
    let lastReservationFailure = null;
    let editor = null;
    let ready = false;
    const status = (message, isError = false) => callbacks.onStatus?.(message, isError);
    const storageWarning = (source, documentId, warnings = [], errors = []) => {
        ...
        status(errors.length > 0
            ...
            : "O diagrama foi recuperado com ajustes de compatibilidade.", errors.length > 0);
    };
    const currentStore = () => {
        if (!editor)
    ...
    const currentFile = () => currentStore().state.doc;
    const currentUi = () => currentStore().state.ui;
    function updateSummary(id, file) {
    ...
    function reserveUniqueDocumentId() {
        lastReservationFailure = null;
        const known = new Set([...summaries.map(({ id }) => id), ...stores.keys()]);
        let seed = "document";
        for (let attempt = 0; attempt < 100; attempt += 1) {
            const candidate = createId();
            if (!isSafeDocumentId(candidate))
                continue;
            seed = candidate;
            if (known.has(candidate))
                continue;
            try {
                if (reserveDocumentId(storage, candidate))
                    return candidate;
            }
            catch {
    ...
    function uniqueInMemoryDocumentId() {
        const known = new Set([...summaries.map(({ id }) => id), ...stores.keys()]);
        for (let attempt = 0; attempt < 100; attempt += 1) {
            const candidate = createId();
            if (!isSafeDocumentId(candidate) || known.has(candidate))
                continue;
            try {
                if (storage.getItem(documentKey(candidate)) !== null)
                    continue;
            }
            catch {
    ...
    function makeStore(id, file, initialDirty = false) {
        return createModel({
            doc: file,
            ...
            persistence: {
                persist(doc) {
                    if (recoverableClaims.get(id) === "unreserved") {
                        if (storage.getItem(documentKey(id)) !== null
    ...
    function activate(id, file) {
        activeId = id;
        editor = stores.get(id) ?? (file ? makeStore(id, file) : null);
        if (!editor)
            throw new Error("document store is unavailable");
        stores.set(id, editor);
        updateSummary(id, editor.state.doc);
        callbacks.onDocuments?.();
        notifyDirty();
    }
    function flush() {
    ...
    function cleanupFailedCreation(id, previousActiveId) {
        try {
    ...
    function createDocument(file = createEmptyDiagramFile(), retainOnPersistenceFailure = false) {
    ...
    function initialize(initialTemplate) {
        if (ready)
            return;
        try {
            ...
            if (migrationId && !migrated.file)
                storage.removeItem(documentKey(migrationId));
            if (migrated.file && migrated.id) {
                activate(migrated.id, migrated.file);
                if (migrated.persistenceFailed)
                    status("Diagrama aberto, mas não foi possível salvar neste navegador", true);
                else if (migrated.backupFailed)
                    status("Diagrama aberto, mas o backup local falhou", true);
                if (migrated.persistenceFailed || migrated.backupFailed) {
                    storageWarning("migration", migrated.id, migrated.backupFailed ? ["legacy backup failed"] : [], migrated.persistenceFailed ? ["legacy persistence failed"] : []);
                }
                ready = true;
                callbacks.onReady?.(true);
                return;
        ...
        if (normalized?.ok)
            createDocument(normalized.file);
        else {
    ...
    function replaceDocumentFile(id, file) {
        const normalized = normalize(file);
        if (!normalized.ok) {
    ...
    function deleteDocumentById(deletedId, fallbackId) {
        if (!flush())
            return false;
    ...
    function reportLimited(field) {
        status(`${field} foi ajustado ao limite suportado pelo formato.`);
    }
    function insertTemplateInto(documentId, input) {
        const store = stores.get(documentId);
        if (!store)
            return false;
        const result = insertIntoDiagram(store.state.doc, input, () => uniqueItemId(store) ?? "");
        if (!result.ok) {
    ...
    function validSelection(selection, store = currentStore()) {
        if (!selection)
            return null;
        if (selection.kind === "node")
            return store.nodeById(selection.id) ? selection : null;
    ...
    const api = {
        ...
        get nodeIndex() {
            return new Map([...currentStore().nodeIndex].map(([id, node]) => [id, clone(node)]));
        },
        ...
        async installCourseDocuments(request) {
            ...
            for (const template of COURSE_TEMPLATES) {
                ...
                if (!existing) {
                    if (!createDocument(loaded.file))
                        return false;
                    continue;
                }
                if (replaceExisting && !replaceDocumentFile(existing.id, loaded.file))
                    return false;
            }
            ...
            for (const retired of summaries.filter(({ title }) => RETIRED_DOCUMENT_TITLES.includes(title))) {
                if (!deleteDocumentById(retired.id, fallbackId))
                    return false;
            }
            try {
            ...
            if (preferredId && preferredId !== activeId)
                switchDocument(preferredId);
            else if (!preferredId && fallbackId && fallbackId !== activeId)
                switchDocument(fallbackId);
            callbacks.onDocuments?.();
            return true;
        },
        switchDocument,
        ...
        connectNodes(fromId, toId, anchors) {
            ...
            const duplicate = currentFile().edges.some((edge) => edge.from === fromId && edge.to === toId &&
            ...
            if (!edgeId)
                return false;
            update((state) => {
                const edge = { id: edgeId, from: fromId, to: toId, label: "" };
                if (fromSide)
        ...
        removeEdgePoint(id, index) {
            update((state) => {
                const edge = state.doc.edges.find((item) => item.id === id);
                if (!edge?.points || index < 0 || index >= edge.points.length)
                    return;
        ...
        reorderNode(id, direction) {
            ...
            update((state) => {
                ...
                if (node)
                    state.doc.nodes.splice(target, 0, node);
            }, "commit");
        },
        finishTransient() {
            update(() => { }, "commit");
        ...
        connectNode(id) {
            ...
            if (!from)
                return void update((state) => { state.ui.connectFrom = id; }, "transient");
            if (from === id)
            ...
            update((state) => {
                if (!exists && edgeId)
                    state.doc.edges.push({ id: edgeId, from, to: id, label: "" });
                state.ui.connectFrom = null;
                state.ui.connectCursor = null;
        ...
        setTitle(title) {
            ...
            if (normalized.length !== (title.trim() || DEFAULT_DIAGRAM_TITLE).length)
                reportLimited("O título");
        ...
        handleExternalStorageChange(documentId) {
            ...
            try {
                ...
                if (!result.file) {
                    ...
                    if (!recovered) {
                        ...
                        if (!persisted && !editor) {
                            ...
                            if (!replacementId)
                                throw new Error("replacement document could not be retained");
                            let claim = "unreserved";
                            try {
                                if (storage.getItem(documentKey(replacementId)) !== null)
                                    claim = "reserved";
                            }
                            catch {
                            ...
                            status("Armazenamento indisponível; o novo diagrama foi preservado para nova tentativa.", true);
                        }
                    }
                    callbacks.onChange?.("ui-only");
                    callbacks.onDocuments?.();
                ...
                if (!isActive) {
                    if (store)
                        stores.set(documentId, makeStore(documentId, result.file));
